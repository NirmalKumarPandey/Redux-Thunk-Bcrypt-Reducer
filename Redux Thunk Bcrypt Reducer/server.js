...
app.post("/login", uploads.none(), async (req, res) => {
    ...
    if (userDetailsArr.length > 0) {
        ...
        let isPaswordCorrect = await bcrypt.compare(req.body.password, userDetailsArr[0].password);
        if (isPaswordCorrect == true) {
...
let connectToTheMongoDB = async () => {
    try {
        await mongoose.connect("mongodb://localhost:27017/REDUX1234");
